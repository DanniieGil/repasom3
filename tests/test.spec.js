...
describe("model ", function () {
  ...
  describe("must have a reset method for resetting the model ", function () {
    ...
    it("must reset clients to {}", function () {
      model.clients = {
        javier: "https://github.com/JavierBalonga",
        comment: "aca me podes heatear tranquilo ;)",
      };
      model.reset();
  ...
  describe("must have an addAppointment method to add appointments to that client", function () {
    ...
    it("must be adding multiple appointments in the order as they are added, and must be handling multiple clients", function () {
      ...
      expect(model.clients.javier[0])
        ...
        .to.be.equal("22/10/2020 14:00");
      model.addAppointment("javier", { date: "22/10/2020 16:00" });
      expect(model.clients.javier[1])
        .to.have.property("date")
        .to.be.equal("22/10/2020 16:00");
      model.addAppointment("alejandro", { date: "22/10/2020 11:00" });
      expect(model.clients.alejandro[0])
        .to.have.property("date")
        .to.be.equal("22/10/2020 11:00");
      model.addAppointment("alejandro", { date: "22/10/2020 12:00" });
      expect(model.clients.alejandro[1])
        .to.have.property("date")
        .to.be.equal("22/10/2020 12:00");
    });
  ...
  describe("must have erase metod for delete appointments", function () {
    ...
    it("it must receive a name and if receive a date must erase the appointment with that date", function () {
      ...
      expect(model.clients.alejandro).to.be.deep.equal([
        { date: "22/10/2020 12:00", status: "cancelled" },
      ]);
  ...
  describe("must have a metod getAppointments, to view the appointments of a client", function () {
    ...
    it("if a status was passed, should only return the appointments with that status", function () {
      ...
      let appointmentsCancelled = model.getAppointments(
        "alejandro",
        "cancelled"
      );
      expect(appointmentsPending).to.be.deep.equal([
        { date: "22/10/2020 14:00", status: "pending" },
      ]);
      expect(appointmentsAttended).to.be.deep.equal([
        { date: "22/10/2020 16:00", status: "attended" },
      ]);
      expect(appointmentsExpired).to.be.deep.equal([
        { date: "22/10/2020 11:00", status: "expired" },
      ]);
      expect(appointmentsCancelled).to.be.deep.equal([
        { date: "22/10/2020 12:00", status: "cancelled" },
      ]);
    });
  });
...
describe("server", () => {
  ...
  describe("POST /api/Appointments", () => {
    ...
    xit("add a appointment to a client", () => {
      return agent
        ...
        .send({
          client: "alejandro",
          appointment: { date: "22/10/2020 11:00" },
        })
        .expect(200)
        .expect(() => {
          expect(model.clients.alejandro).to.be.deep.equal([
            { date: "22/10/2020 11:00", status: "pending" },
          ]);
        });
    });
    xit("responds the appointment after the addition", () => {
      return agent
        ...
        .send({
          client: "alejandro",
          appointment: { date: "22/10/2020 12:00" },
        })
        .expect(200)
        .expect((res) => {
          expect(res.body).to.be.deep.equal({
            date: "22/10/2020 12:00",
            status: "pending",
          });
        });
    });
  ...
  describe("GET /api/Appointments/:name?date=xxx&option=xxx", () => {
    xit("responds with a status 400 (bad request) and a string message, if the client does not exist", () => {
      return agent
        .get("/api/Appointments/pepe?date=22/10/2020%2014:00&option=attend")
        .expect(400)
        .expect((res) => {
    ...
    xit("responds with a status 400 (bad request) and a string message, if the client does not have a appointment for this date", () => {
      return agent
        .get("/api/Appointments/javier?date=23/10/2020%2014:00&option=attend")
        .expect(400)
        .expect((res) => {
          expect(res.text).to.be.equal(
            "the client does not have a appointment for that date"
          );
        });
    });
    xit("responds with a status 400 (bad request) and a string message, if the option is not attend, expire or cancel", () => {
      return agent
        .get(
          "/api/Appointments/javier?date=22/10/2020%2014:00&option=wrongOption"
        )
        .expect(400)
        .expect((res) => {
          expect(res.text).to.be.equal(
            "the option must be attend, expire or cancel"
          );
        });
    });
    xit("attend a appointment if the option passed by query is `attend`", () => {
      return agent
        .get("/api/Appointments/javier?date=22/10/2020%2014:00&option=attend")
        .expect(200)
        .expect((res) => {
          expect(model.clients.javier[0]).to.be.deep.equal({
    ...
    xit("expire a appointment if the option passed by query is `expire`", () => {
      return agent
        .get("/api/Appointments/javier?date=22/10/2020%2016:00&option=expire")
        .expect(200)
        .expect((res) => {
          expect(model.clients.javier[1]).to.be.deep.equal({
            date: "22/10/2020 16:00",
            status: "expired",
          });
        });
    });
    xit("cancel a appointment if the option passed by query is `cancel`", () => {
      return agent
        .get("/api/Appointments/javier?date=22/10/2020%2014:00&option=cancel")
        .expect(200)
        .expect((res) => {
          expect(model.clients.javier[0]).to.be.deep.equal({
            date: "22/10/2020 14:00",
  ...
  describe("GET /api/Appointments/:name/erase", function () {
    xit("responds with a status 400 (bad request) and a string message, if the client does not exist", () => {
      return agent
        .get("/api/Appointments/pepe/erase?date=22/10/2020%2014:00")
        .expect(400)
        .expect((res) => {
          expect(res.text).to.be.equal("the client does not exist");
        });
    ...
    xit("erase a appointment", () => {
      return agent
        .get("/api/Appointments/javier/erase?date=22/10/2020%2014:00")
        .expect(200)
        .expect((res) => {
          expect(model.clients.javier).to.be.deep.equal([
            { date: "22/10/2020 16:00", status: "pending" },
    ...
    xit("erase all appointments of a certain status", () => {
      model.expire("javier", "22/10/2020 14:00");
      return agent
        .get("/api/Appointments/javier/erase?date=expired")
        .expect(200)
        .expect((res) => {
          expect(model.clients.javier).to.be.deep.equal([
            { date: "22/10/2020 16:00", status: "pending" },
    ...
    xit("responds the array of erased appointments", () => {
      ...
      return agent
        ...
        .expect((res) => {
          expect(res.body).to.be.deep.equal([
            { date: "22/10/2020 14:00", status: "expired" },
          ]);
        });
    });
  });
  ...
  describe("GET /api/Appointments/getAppointments/:name", function () {
    xit("responds with the array of appointments with that status", () => {
      return agent
        .get("/api/Appointments/getAppointments/javier?status=pending")
        .expect(200)
        .then((res) => {
          expect(res.body).to.be.deep.equal([
            { date: "22/10/2020 14:00", status: "pending" },
